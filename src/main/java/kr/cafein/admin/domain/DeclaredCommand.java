...
public class DeclaredCommand {
	...
	private int d_seq;			//신고글의 시퀀스
	@NotEmpty
	private int d_target_path; 	//신고글 경로(target_type :  - 0프랜차이즈 댓글 1- 개인카페 댓글 2- 커스터마이징 댓글)
	@NotEmpty
	private int d_target_num;  // 신고 대상(댓글)의 상위 경로 시퀀스
	@NotEmpty
	private int d_target_id; 	//신고한 댓글 (seq)   pk
	
	@NotEmpty
	private String d_mem_id;  		//신고를 한 회원 아이디(seq) pk
	private String d_target_mem_id; //신고 당한 회원아이디(seq) 혹은 게스트 (Null:Guest, Not Null:회원)
	private Date d_reg_date;
	private String d_content;
	private int d_state;		 //처리상태(0.처리 전, 1.처리 중,2.처리 완료, 3처리 보류, 4. 처리 취소)
	private String d_comment; // 처리 코멘트
	...
	public String toString() {
		return "DeclaredCommand [d_seq=" + d_seq + ", d_target_path=" + d_target_path + ", d_target_num=" + d_target_num
				+ ", d_target_id=" + d_target_id + ", d_mem_id=" + d_mem_id + ", d_target_mem_id=" + d_target_mem_id
				+ ", d_reg_date=" + d_reg_date + ", d_content=" + d_content + ", d_state=" + d_state + ", d_comment="
				+ d_comment + "]";
	}
