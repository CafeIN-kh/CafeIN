...
public interface AdminFranchiseChartMapper {
	...
	@Select("SELECT to_char(ucnt_log_reg_date,'DD') reg_date,sum(ucnt_log_franchise) ucnt_log_franchise FROM user_count_log WHERE to_char(ucnt_log_reg_date,'YYYYMMDD') BETWEEN to_char(sysdate-7,'YYYYMMDD') AND to_char(sysdate,'YYYYMMDD') GROUP BY to_char(ucnt_log_reg_date,'DD') ORDER BY to_char(ucnt_log_reg_date,'DD') ASC")
	...
	@Select("SELECT to_char(like_reg_date,'DD') like_reg_date , count(*) count FROM u_like WHERE franchise_num != 0 AND franchise_num is not null AND to_char(like_reg_date,'YYYYMMDD') BETWEEN to_char(sysdate-7,'YYYYMMDD') AND to_char(sysdate,'YYYYMMDD') GROUP BY to_char(like_reg_date,'DD') ORDER BY like_reg_date ASC")
	public List<AdminFranchiseLikeCommand> franchiseLikeDayCount();
	@Select("SELECT to_char(like_reg_date,'DD') like_reg_date , count(*) count FROM u_like WHERE fmenu_num != 0 AND fmenu_num is not null AND to_char(like_reg_date,'YYYYMMDD') BETWEEN to_char(sysdate-7,'YYYYMMDD') AND to_char(sysdate,'YYYYMMDD') GROUP BY to_char(like_reg_date,'DD') ORDER BY like_reg_date ASC")
	...
	@Select("SELECT to_char(d_reg_date,'DD') reg_date,count(*) count FROM declared WHERE to_char(d_reg_date,'YYYYMMDD') BETWEEN to_char(sysdate-7,'YYYYMMDD') AND to_char(sysdate,'YYYYMMDD') GROUP BY to_char(d_reg_date,'DD') ORDER BY to_char(d_reg_date,'DD') ASC")
