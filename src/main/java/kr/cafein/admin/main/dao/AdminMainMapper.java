...
public interface AdminMainMapper {
	...
	@Select("SELECT to_char(like_reg_date,'DD') like_reg_date , count(*) count FROM u_like WHERE franchise_num != 0 AND franchise_num is not null AND to_char(like_reg_date,'YYYYMMDD') BETWEEN to_char(sysdate-20,'YYYYMMDD') AND to_char(sysdate,'YYYYMMDD') GROUP BY to_char(like_reg_date,'DD') ORDER BY like_reg_date ASC")
	public List<AdminMainCountCommand> franchiseCount();
	@Select("SELECT to_char(like_reg_date,'DD') like_reg_date , count(*) count FROM u_like WHERE pcafe_num != 0 AND pcafe_num is not null AND to_char(like_reg_date,'YYYYMMDD') BETWEEN to_char(sysdate-20,'YYYYMMDD') AND to_char(sysdate,'YYYYMMDD') GROUP BY to_char(like_reg_date,'DD') ORDER BY like_reg_date ASC")
	public List<AdminMainCountCommand> privateCount();
	@Select("SELECT to_char(like_reg_date,'DD') like_reg_date , count(*) count FROM u_like WHERE custom_num != 0 AND custom_num is not null AND to_char(like_reg_date,'YYYYMMDD') BETWEEN to_char(sysdate-20,'YYYYMMDD') AND to_char(sysdate,'YYYYMMDD') GROUP BY to_char(like_reg_date,'DD') ORDER BY like_reg_date ASC")
	public List<AdminMainCountCommand> customCount();
	@Select("SELECT to_char(like_reg_date,'DD') like_reg_date , count(*) count FROM u_like WHERE pmenu_num != 0 AND pmenu_num is not null AND to_char(like_reg_date,'YYYYMMDD') BETWEEN to_char(sysdate-20,'YYYYMMDD') AND to_char(sysdate,'YYYYMMDD') GROUP BY to_char(like_reg_date,'DD') ORDER BY like_reg_date ASC")
	public List<AdminMainCountCommand> pmenuCount();
	@Select("SELECT to_char(like_reg_date,'DD') like_reg_date , count(*) count FROM u_like WHERE fmenu_num != 0 AND fmenu_num is not null AND to_char(like_reg_date,'YYYYMMDD') BETWEEN to_char(sysdate-20,'YYYYMMDD') AND to_char(sysdate,'YYYYMMDD') GROUP BY to_char(like_reg_date,'DD') ORDER BY like_reg_date ASC")
