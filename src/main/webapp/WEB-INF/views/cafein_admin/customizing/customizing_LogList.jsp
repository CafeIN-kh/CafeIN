 ...
 <section id="content">
	...
	<div class="card">
		<div class="card-header" style="padding-bottom: 0px;">
			<h2>
				Customizing <small>Log List</small>
		...
		<table id="data-table-command"
			...
			<thead>
				<tr>     
					...
					<th data-column-id="an_log_uid">User id</th>
			...
			<tbody>
				<c:forEach var="list" items="${list }">
					<tr>
						<td>${list.c_log_seq }</td>
						<td>${list.u_uid}</td>
						<td>${list.c_log_message }</td>
						<td>${list.c_log_reg_date}</td>
						<c:choose>
							<c:when test="${list.c_log_change==0}">
								<td>Write</td>
							</c:when>
							<c:when test="${list.c_log_change==1}">
								<td>Delete</td>
							</c:when>
							<c:otherwise>
								<td>Update</td>
