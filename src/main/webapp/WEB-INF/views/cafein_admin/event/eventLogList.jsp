		...
		<table id="data-table-command"
			...
			<tbody>
				<c:forEach var="list" items="${list }">
					<tr>
						<td>${list.e_log_num }</td>
						<td>${list.e_log_uid}</td>
						<td>${list.e_log_message }</td>
						<td>${list.e_log_reg_date}</td>
						<c:choose>
							<c:when test="${list.e_log_change==0}">
								<td>Write</td>
							</c:when>
							<c:when test="${list.e_log_change==1}">
								<td>Delete</td>
							</c:when>
							<c:otherwise>
								<td>Update</td>
